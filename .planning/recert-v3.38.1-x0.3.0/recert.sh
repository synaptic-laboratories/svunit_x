#!/usr/bin/env bash
# recert.sh - six-target sign-off for v3.38.1-x0.3.0.
#
# Structurally identical to .planning/phases/04-xilinx-vivado-xsim-integration/
# 04-reproduce.sh; differs only in artefacts root, phase dir, and comment
# header.  Uses ARTEFACTS_ROOT =
# /srv/share/repo/.../r_v3_38_1_x0_3_0_artefacts so the sign-off identity
# binds to the x0.3.0 qualifiedVersion baked into nix/mk-certify.nix.
#
# Usage:
#   bash .planning/recert-v3.38.1-x0.3.0/recert.sh [--manifest PATH] [--performance PATH]
#   bash .planning/recert-v3.38.1-x0.3.0/recert.sh --reuse-session SESSION_STAMP
#
# Output:
#   - .planning/recert-v3.38.1-x0.3.0/sign-off-manifest.tsv
#   - .planning/recert-v3.38.1-x0.3.0/performance-summary.tsv
#   - .planning/recert-v3.38.1-x0.3.0/recert-<session>.log
#
# Exit code 0 iff all six targets PASS jq-semantically and the Vivado smoke
# evidence is present in the Vivado target output directory.

set -euo pipefail

ARTEFACTS_ROOT="/srv/share/repo/sll/g_sll_infra/g_sll_infra_dev_001/g_ext_tools_qualified/g_svunit_x/r_v3_38_1_x0_3_0_artefacts"
LICENSE_DIR="/srv/share/repo/sll/g_sll_poc/g_2026/ContainerPlayPen/launch"
PHASE_DIR=".planning/recert-v3.38.1-x0.3.0"
DEFAULT_MANIFEST="$PHASE_DIR/sign-off-manifest.tsv"
DEFAULT_PERFORMANCE="$PHASE_DIR/performance-summary.tsv"
...
mkdir -p "$PHASE_DIR"
RUN_LOG="$PHASE_DIR/recert-${SESSION_STAMP}.log"
touch "$RUN_LOG"
